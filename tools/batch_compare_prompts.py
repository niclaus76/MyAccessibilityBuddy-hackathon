...
Configuration:
Test parameters are defined in backend/config/config.advanced.json under:
  - testing.batch_comparison.prompts: List of prompt files to compare
  - testing.batch_comparison.test_images: Images to process
  - testing.batch_comparison.model: LLM provider and model settings
  - testing.folders: Input/output folder paths
...
Example Configuration (config.advanced.json):
  "testing": {
    "batch_comparison": {
      "prompts": [
...
def run_batch_processing():
    ...
    try:
        # Build command with test folders
        cmd = [
            "python3", "app.py", "-p",
